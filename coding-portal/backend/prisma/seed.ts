...
type StarterCodes = Record<string, string>;

type GeneratedCase = { input: string; output: string };
...
type ProblemTemplate = {
  ...
  constraints: string;
  explanation: string;
  makeCase: (seed: number) => GeneratedCase;
};

const starterCodes: StarterCodes = {
...
function makeArray(seed: number, n: number, min = -20, max = 50): number[] {
  const next = lcg(seed);
...
function minStepsToOne(n: number): number {
...
function nextGreaterToLeft(arr: number[]): number[] {
  const st: number[] = [];
  const out: number[] = [];
  for (const v of arr) {
    while (st.length && st[st.length - 1] <= v) st.pop();
...
function isBalancedBrackets(s: string): boolean {
  const st: string[] = [];
  const open = new Set(["(", "[", "{"]);
  ...
  for (const ch of s) {
    if (open.has(ch)) st.push(ch);
      ...
      if (!st.length || st.pop() !== closeMap[ch]) return false;
    }
  }
  return st.length === 0;
}

function buildGraph(seed: number, n: number, m: number): Array<[number, number]> {
  const next = lcg(seed);
...
function countComponents(n: number, edges: Array<[number, number]>): number {
  ...
  for (let i = 1; i <= n; i++) roots.add(find(i));
  return roots.size;
}

function countTreeLeaves(parents: number[]): number {
  ...
  let leaves = 0;
  for (let i = 0; i < n; i++) {
    if (!hasChild[i]) leaves++;
  }
  return leaves;
}

function makeRandomString(seed: number, length: number): string {
  const next = lcg(seed);
...
const templates: ProblemTemplate[] = [
  ...
  },
  {
    ...
    makeCase: (seed) => {
      ...
      const k = 4 + (seed % 5);
      const out: string[] = [];
      for (let i = 0; i < k; i++) out.push(words[randInt(next, 0, words.length - 1)]);
  ...
  },
  {
    ...
    makeCase: (seed) => {
      ...
      let degree = 0;
      for (const [u, v] of edges) if (u === x || v === x) degree++;
  ...
  },
  {
    ...
    makeCase: (seed) => {
      ...
      for (const key of arr) {
        let idx = key % 10;
        while (table[idx] !== -1) idx = (idx + 1) % 10;
        table[idx] = key;
      }
...
function buildProblems() {
  ...
  for (const template of templates) {
    ...
    const langStarterCodes: StarterCodes = {};
...
async function seedProblemsAndJourneys() {
  ...
  const hasTag = (problemTags: unknown, tag: string) =>
    Array.isArray(problemTags) && (problemTags as string[]).some((t) => t.toLowerCase() === tag.toLowerCase());
