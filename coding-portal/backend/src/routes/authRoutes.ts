...
function verifySsoToken(token: string): { payload: Record<string, any> | null; reason?: string } {
  ...
  try {
    ...
    if (!parsed?.email) return { payload: null, reason: "missing_email" };
    if (!parsed?.name) return { payload: null, reason: "missing_name" };
    if (!parsed?.exp) return { payload: null, reason: "missing_exp" };
...
router.get("/me", requireAuth, async (req, res, next) => {
  try {
    const user = await prisma.user.findUnique({
      ...
      select: { id: true, name: true, firstName: true, lastName: true, studentId: true, profession: true, bio: true, email: true, role: true, createdAt: true, profileImage: true, phone: true, college: true, branch: true, graduationYear: true, skills: true, location: true, linkedin: true, github: true, buildupUid: true }
    });
    if (!user) {
