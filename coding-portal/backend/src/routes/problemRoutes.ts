import { Difficulty, Language, Prisma } from "@prisma/client";
...
router.get("/", async (req, res, next) => {
  try {
    ...
    if (q) where.OR = [{ title: { contains: q } }, { statement: { contains: q } }];
    ...
    if (tag) {
      ...
      problems = problems.filter(
        (p) => Array.isArray(p.tags) && (p.tags as string[]).some((t) => t.toLowerCase().includes(lowTag) || lowTag.includes(t.toLowerCase()))
      );
    }

    if (language) {
      problems = problems.filter((p) => Array.isArray(p.supportedLanguages) && (p.supportedLanguages as string[]).includes(language));
    }
