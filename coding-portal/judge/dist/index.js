...
function runProcess(command, args, cwd, stdin = "", timeoutMs = 2000) {
...
async function writeWorkspace(language, sourceCode) {
    const dir = path.join(os.tmpdir(), `judge-${randomUUID()}`);
    await fs.mkdir(dir, { recursive: true });
    const fileName = language === "C"
        ? "main.c"
        : language === "CPP"
            ? "main.cpp"
            : language === "PYTHON"
...
async function compileIfNeeded(language, dir, fileName) {
    if (language === "PYTHON")
...
async function runCase(language, dir, input, timeoutMs) {
    if (language === "PYTHON") {
        const exists = await commandExists("python");
        if (!exists)
            return { code: null, stdout: "", stderr: "", timedOut: false, spawnError: "python runtime not installed" };
        return runProcess("python", ["main.py"], dir, input, timeoutMs);
    }
    if (language === "JAVA") {
        const exists = await commandExists("java");
        if (!exists)
            return { code: null, stdout: "", stderr: "", timedOut: false, spawnError: "java runtime not installed" };
